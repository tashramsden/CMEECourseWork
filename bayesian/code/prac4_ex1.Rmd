...
```{r}
## The chains have converged - "hairy caterpillar"! - w diff starting params,
## still get convergence to these values
```
...
```{r}
## They are efficient - not too much autocorr
```
...
```{r}
## Neither plot looks good - ie inefficient for both params - neither has happily settled - no hairy caterpillars!
...
```{r}
## try plotting mcmc4 (takes a while)

## when running for longer -> chains do converge
...
## dk.mcmc and dk.mcmc.b both have same (good) step sizes - diff starting val for k - still both converge
hist( x = dk.mcmc3.b$k, prob=TRUE, breaks=bks, border=NA,
      col=rgb(0, 0, 1, .5), las=1, xlab="kappa",
      xlim=c(0,100), ylim=c(0,.055))
hist( x = dk.mcmc3$k, prob=TRUE, breaks=bks, border=NA,
      col=rgb(.5, .5, .5, .5), add=TRUE)
## dk.mcmc3 and dk.mcmc3.b both have same (bad) step sizes - w diff starting params now -not so convergent
```
